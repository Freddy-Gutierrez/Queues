...
//--------------------------------------------------------------------------------------------------------------------------------------              
    public ArrayQueue(){
       ...
       T[] temp = (T[]) new Object[MIN_DEFAULT_SIZE];                   //Constructor that creates our array with the default initial size, changes initialized to true, 
...
//--------------------------------------------------------------------------------------------------------------------------------------    
    ...
    public T dequeue() {
       if(!isEmpty()){
          data = myArray[0];                                     //Checks to see if there are elements in the queue to remvoe
          @SuppressWarnings("Unchecked")                         //if there are, returns first element in the array, then shifts
          T[] temp = (T[]) new Object[myArray.length];           //all remaining elements to the left by one, and decrements the 
          int newIndex = 0;                                      // number of entries.
          for(int i = 1; i<myArray.length; i++){
             temp[newIndex] = myArray[i];  
             newIndex++;
          }
          myArray = temp;
          numEntries--;
